...
{
	//printf("HELLO I AM IN MPIDI_PG 2\n");
    int mpi_errno = MPI_SUCCESS;
...
{
	//printf("HELLO I AM IN MPIDI_PG 4\n");
    MPIDI_PG_t * pg_prev;
    MPIDI_PG_t * pg_cur;
    int i;
    int mpi_errno = MPI_SUCCESS;
...
{
	printf("HELLO I AM IN MPIDI_PG 5\n");
    MPIDI_PG_t * pg;
    int mpi_errno = MPI_SUCCESS;
...
{
	printf("HELLO I AM IN MPIDI_PG 6\n");
...
{
	printf("HELLO I AM IN MPIDI_PG 7\n");
    *pg_ptr = (*iter);
    if ((*iter) != NULL) {
        (*iter) = (*iter)->next;
    }
...
{
	printf("HELLO I AM IN MPIDI_PG 8\n");
    return (*iter != NULL);
}
...
{
	printf("HELLO I AM IN MPIDI_PG 9\n");
    *iter = MPIDI_PG_list;
    return MPI_SUCCESS;
}
...
{
	printf("HELLO I AM IN MPIDI_PG 10\n");
    int mpi_errno = MPI_SUCCESS;
...
int MPIDI_PG_Create_from_string(const char * str, MPIDI_PG_t ** pg_pptr,
				int *flag)
{
	printf("HELLO I AM IN MPIDI_PG 11\n");
    int mpi_errno = MPI_SUCCESS;
    const char *p;
    int vct_sz;
...
{
	printf("HELLO I AM IN MPIDI_PG 12\n");
...
{
//	printf("In MPIDI_PG_SetConnInfo\n");
#ifdef USE_PMI2_API
    int mpi_errno = MPI_SUCCESS;
    int len;
 ...
 fn_fail:
	...
	MPIR_ERR_SETANDJUMP1(mpi_errno,MPI_ERR_OTHER, "**snprintf",
		...
		//printf("In SetInfo, calling KVS_PUT with rank = %d\nkey = %s\nconnString = %s\n", rank, key, connString);
...
{
	//printf("In GetConnInfoKVS in ch3\n");
#ifdef USE_PMI2_API
		printf("yea we're using PMI2\n");
	...
	printf("About to call PMI2_KVS_GET\n");
	mpi_errno = PMI2_KVS_Get(pg->connData, PMI2_ID_NULL, key, buf, bufsize, &vallen);
  printf("buf = %s\n", buf);
				if (mpi_errno) MPIR_ERR_POP(mpi_errno);
    }
 ...
 fn_fail:
    ...
    MPID_THREAD_CS_ENTER(POBJ, MPIR_THREAD_POBJ_PMI_MUTEX);
		//printf("About to call PMI_KVS_GET\n");
		printf("key = %s\n", key);
...
{
	printf("HELLO I AM IN MPIDI_PG 14\n");
    char *string = 0;
			      ...
			      MPIDI_PG_t *pg ATTRIBUTE((unused)) )
{
    /* Fixme: this should be a failure to call this routine */
		printf("HELLO I AM IN MPIDI_PG 15\n");
    return MPI_SUCCESS;
}
static int connFreeKVS( MPIDI_PG_t *pg )
{
	//printf("HELLO I AM IN MPIDI_PG 16\n");
    if (pg->connData) {
	MPIU_Free( pg->connData );
    }
    return MPI_SUCCESS;
...
{
	//printf("In MPIDI_PG_InitConnKVS\n");
#ifdef USE_PMI2_API
    int mpi_errno = MPI_SUCCESS;
...
{
	//printf("HELLO I AM IN MPIDI_PG 18\n");
...
{
	printf("HELLO I AM IN MPIDI_PG 19\n");
    int mpi_errno = MPI_SUCCESS;
    char *str = NULL, *pg_id;
    int  i, len=0;
    MPIU_CHKPMEM_DECL(1);
...
{
	printf("HELLO I AM IN MPIDI_PG 20\n");
...
{
	printf("HELLO I AM IN MPIDI_PG 21\n");
    MPIDI_ConnInfo *conninfo = (MPIDI_ConnInfo *)pg->connData;
    int i;
...
{
	printf("HELLO I AM IN MPIDI_PG 22\n");
    int pg_size, i;
...
int MPIDI_PrintConnStrToFile( FILE *fd, const char *file, int line,
			      const char *label, const char *str )
{
	printf("HELLO I AM IN MPIDI_PG 23\n");
    int pg_size, i;
...
{
	printf("IN GETCONNSTRING\n");
    int mpi_errno = MPI_SUCCESS;

    if (pg->getConnInfo) {
			//printf("ABOUT TO CALL PG->GETCONNINFO\n");
...
{
	printf("HELLO I AM IN MPIDI_PG 24\n");
    MPIDI_VC_t *vc;
...
{
	//printf("HELLO I AM IN MPIDI_PG 25\n");
...
{
	printf("HELLO I AM IN MPIDI_PG 26\n");
    MPIDI_PG_t *pg;
    int         i;
...
{
printf("HELLO I AM IN MPIDI_PG 27\n");
#ifdef USE_PMI2_API
