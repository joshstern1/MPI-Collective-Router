                                       ...
                                       MPID_Comm **mapping_comm)
{
  ...
  MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    ...
    */
    ...
    /*  if(world_rank==0){
        printf("new communicator with mapping:\n");
      }*/
        int wsize;
        subsetOfWorld = 1;
...
#           endif
      ...
      if(world_rank==0){
        printf("It is not a subsetofWorld\n");
      }
        for (i=0; i<n; i++) {
            ...
            for (j=0; j<comm_ptr->local_size; j++) {
                ...
                if (comm_lpid == group_ptr->lrank_to_lpid[i].lpid) {
                    mapping[i] = j;
                    printf("mapping for %d is %d, comm_lpid = %d\n", world_rank, j, comm_lpid);
                    break;
                }
            }
                           ...
                           MPID_Comm **newcomm_ptr)
{
  int world_rank;
  MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
  //  printf("In MPIR_COMM_CREATE_INTRA, %d\n", world_rank);
    //MPIDI_VC_t * myvct;
    int mpi_errno = MPI_SUCCESS;
...
*/
    ...
    if (group_ptr->rank != MPI_UNDEFINED) {
        ...
        //printf("About to do MPIR_Comm_create_calculate_mapping, %d\n", world_rank);
        ...
        /* Setup the communicator's network address mapping.  This is for the remote group,
          ...
          // printf("about to do Comm_create_map, %d\n", world_rank);
        ...
        //printf("about to do Comm_commit, %d\n", world_rank);
        /*if(world_rank==0){
          int i;
          for(i=0; i<n; i++){
            printf("intramapping[%d] = %d\n", i, mapping[i]);
          }
        }*/
...
fn_exit:
    ...
    josh_hook(*newcomm_ptr, mapping, new_context_id);

    if (mapping)
        //printf("about to do MPIU_Free, %d\n", world_rank);
        MPIU_Free(mapping);
