...
{
    //printf("In MPIDI_CH3_Init, nemesis\n");
    int mpi_errno = MPI_SUCCESS;
    int i;
...
int MPIDI_CH3_InitCompleted(void)
{
 ...
 fn_exit:
    ...
    josh_hook(comm_ptr ,NULL, comm_ptr->context_id);
    ...
    if(world_rank == 0){
      ...
      for(i=0; i<world_size; i++){
        char joshbuf[1024];
        int josh = MPIDI_PG_GetConnString( MPIDI_CH3I_my_pg, i, joshbuf, 1024);
        printf("P%d-businesscard = %s\n", i, joshbuf);
        /*char bcardtest[1024];
        int z = MPIDI_CH3_Get_business_card(i, bcardtest, 1024);* //will only give business card for my rank
      }

      MPIU_Context_id_t contextid;
      contextid = comm_ptr->context_id;
      printf("context_id = %d\n", contextid);
      /*MPID_Comm_get_ptr(MPI_COMM_SELF, comm_ptr);
      contextid = comm_ptr->context_id;
      printf("context_id_self = %d\n", contextid);
    }*/
...
static int getportandip(char *str, char **port, char **ip){

  char portstring[] = "port";
  char ipstring[] = "ifname";
  *port = strstr(str, portstring)+5;
  *ip = strstr(str, ipstring)+7;
  int i=0;
  char c = *(*port);
  while(c!='$'){
    i++;
...
void josh_hook(MPID_Comm *newcomm_ptr, int *mapping, MPIU_Context_id_t new_context_id){
    int world_rank, size, rank, index, children, lgcommsize, root, i;
    MPI_Comm_rank(MPI_COMM_WORLD, &world_rank);
    size = newcomm_ptr->local_size;
    rank = newcomm_ptr->rank;
    if((world_rank==0)&&(mapping == NULL)){
      ...
      for(i=0; i<size; i++){
        ...
        char *port = joshbuf;
        char *ip = joshbuf;
        int x = getportandip(joshbuf, &port, &ip);
        printf("P%d-businesscard: port = %s, ip = %s\n", i, port, ip);
      }
      //root, local rank, children, lg_commsize
    }
    else if (mapping != NULL){
      if(world_rank==0){
        printf("New communicator with contextid = %d\n", new_context_id);
      }
      children = 0;
      lgcommsize = 0;
      root = mapping[0];
      ...
      for(i=size/2; i>0; i=i/2){
        ...
        index = rank ^ i;
        printf("%d(%d): P%d\n", rank, world_rank, mapping[index]);
        lgcommsize++;
      }
      printf("%d(%d): children = %d\n", rank, world_rank, children);
      if(rank==0){
        printf("%d(%d) is root, lgcommsize = %d\n", rank, world_rank, lgcommsize);
      }
        //char joshbuf[1024];
        //int josh = MPIDI_PG_GetConnString( MPIDI_CH3I_my_pg, mapping[index], joshbuf, 1024);
        //char *port = joshbuf;
        //char *ip = joshbuf;
        //int x = getportandip(joshbuf, &port, &ip);
        //printf("For %d(%d): P%d-businesscard: port = %s, ip = %s\n", rank, world_rank, mapping[index], port, ip);
