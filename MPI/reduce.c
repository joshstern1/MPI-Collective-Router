...
printf("QX in the same file reduce.c, but jumps into MPIR_Reduce_intra for intra communicator reduce\n");
    int mpi_errno = MPI_SUCCESS;
    int mpi_errno_ret = MPI_SUCCESS;
...
printf("QX still in src/mpi/coll/reduce.c, but in MPIR_Reduce_impl()\n");
int mpi_errno = MPI_SUCCESS;
...
int MPI_Reduce(const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
	       MPI_Op op, int root, MPI_Comm comm)
{
printf("QX in src/mpi/coll/reduce.c !!!!!!!!!!!\n");
