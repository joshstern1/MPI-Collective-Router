...
/*
	*|63   	|62       	| 61-59  |58-56   |55-52  	|51  	|50-43|42-40   |39-37|36-34|33-32|31-0   |
...
assign spy_lock = (nextPacket==0)? lock: ((nextIndex == packetIndex)? lock : reduction_table[nextIndex][WaitPos+WaitWidth-1:WaitPos]);
...
*/
add A1(
  .a({10'b0000,reduction_table[packetIndex][PayloadLen-1:0]}),
  .b({10'b000, packetA[PayloadLen-1:0]}),
  .clk(clk),
  .s(sum)
);
...
*/
/*
the following module performs a logic or-ing of the two inputs.
...
the following module performs a logic and-ing of the two inputs.
the results are passed to a pipeline shifter.
...
the following module performs a bitwise or-ing of the two inputs.
the results are passed to a pipeline shifter.
...
the following module performs a bitwise and-ing of the two inputs.
the results are passed to a pipeline shifter.
...
assign reduction_table_entry= (done)?reduction_table[done_index]:0;
assign Outpacket[ValidBitPos] = (done)? 1: 0;
assign Outpacket[ReductionBitPos] = (done)? 1: 0;
...
always@(posedge clk) begin
  ...
  */
  ...
  was not just completed and the incoming packet is not a repeat, update the reduction table with information
  from the incoming header and and wait for the reduction operation result from the ip core
  */
  ...
  */
  ...
  begin
    ...
    2'b00: reduction_table[i][PayloadLen-1:0] <= sum[21:0];
